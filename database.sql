...
-- Bu faylı MySQL veritabanına import edin

-- Veritabanını yarat (əgər mövcud deyilsə)
...
-- Trigger yarad (sifariş yaradıldıqda balansı azaltmaq üçün)
DELIMITER $$
...
BEGIN
    ...
    VALUES (NEW.user_id, 'order_payment', NEW.price, CONCAT('Sifariş #', NEW.id, ' üçün ödəniş'), 'completed');
END$$
DELIMITER ;

-- Trigger yarad (sifariş ləğv edildikdə balansı geri qaytarmaq üçün)
DELIMITER $$
...
BEGIN
    IF OLD.status != 'cancelled' AND NEW.status = 'cancelled' THEN
        ...
        INSERT INTO transactions (user_id, type, amount, description, status)
        VALUES (NEW.user_id, 'deposit', NEW.price, CONCAT('Sifariş #', NEW.id, ' ləğv edildi - geri qaytarıldı'), 'completed');
    END IF;
END$$
DELIMITER ;
...
-- Veritabanı strukturunu yoxlamaq üçün view yarad
...
-- İstifadəçi statistikaları üçün view yarad
...
GROUP BY u.id, u.username, u.telegram_id, u.balance, u.created_at;

-- Veritabanı yaradıldı mesajı
SELECT 'SMM Panel veritabanı uğurla yaradıldı!' as message;